...
from utils import ( Analysis, Problem, Reflection,
                   Solution, 
    tokenizer,
    model,
                   check_correctness, format_example,
...
async def zero_shot_solver(
    problem: Problem, 
    # model: str = FAST_LLM, 
    temperature: float = 0.0, timeout: int = 10
) -> dict:
...
async def rag_solver(
    ...
    rag_result = await rag_solution(problem, solution, temperature, timeout)
